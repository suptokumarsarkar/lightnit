...
class DashboardController extends Controller
{
    ...
    public function paymentSettingsPost(Request $request)
    {
        if ($request->data['paypalClientId']) {
		 ...
		 if ($request->data['paypalClientSecret']) {
            if ($rk = WebSetting::WHERE("key", 'paypalClientSecret')->first()) {
                $rk->value = $request->data['paypalClientSecret'];
                $rk->save();
            } else {
                $rk = new WebSetting;
                $rk->key = "paypalClientSecret";
                $rk->value = $request->data['paypalClientSecret'];
                $rk->save();
            }
        }
		
		 if ($request->data['paypalappId']) {
            if ($rk = WebSetting::WHERE("key", 'paypalappId')->first()) {
                $rk->value = $request->data['paypalappId'];
                $rk->save();
            } else {
                $rk = new WebSetting;
                $rk->key = "paypalappId";
		 ...
		 if ($request->data['paypalmode']) {
            if ($rk = WebSetting::WHERE("key", 'paypalmode')->first()) {
                $rk->value = $request->data['paypalmode'];
                $rk->save();
            } else {
                $rk = new WebSetting;
                $rk->key = "paypalmode";
	...
	public function invoices(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $invoices = Invoice::where(function ($q) use ($key) {
                foreach ($key as $value) {
                    $q->orWhere('users.name', 'like', "%{$value}%")
                        ->orWhere('users.email', 'like', "%{$value}%")
                        ->orWhere('plans.name', 'like', "%{$value}%")
                        ->orWhere('invoices.amount', 'like', "%{$value}%");
                }
            });
        ...
        $invoices = $invoices->select('invoices.*','users.email','users.name','plans.name as plan_name')
		->leftJoin('users', 'users.id', '=', 'invoices.user_id')
		->leftJoin('plans', 'plans.id', '=', 'invoices.plan_id')
		->latest()->paginate(Helpers::getPagination())->appends($query_param);
        return view('admin-views.invoices.index', compact('invoices', 'search'));
    }
