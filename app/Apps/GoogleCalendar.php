...
// Real API
    ...
    public function updateEvent($access_token, array $params)
    {
        ...
        $eventData = $this->getEvent($access_token, $param);
	
        if(isset($eventData['attendees'])){
            $params["attendees"] = $eventData['attendees'];
        } 
        if (isset($param['attendees'])) {
			
			if(is_array($param['attendees'])){
				foreach ($param['attendees'] as $email) {
					$params["attendees"][] = ['email' => $email];
				}
			} else {
				   $params["attendees"][] = ['email' => $param['attendees']];
			}
    ...
    {
        $url = "https://www.googleapis.com/calendar/v3/calendars/{$params['calenderId']}/events?orderBy=updated&access_token=" . $access_token;
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $response = curl_exec($ch);
        return json_decode($response, 1)['items'] ?? [];
