...
class Manager extends Controller
{
    ...
    public function checkTrigger(Request $request)
    {
		...
		$conditions = isset($request->conditions)?json_decode($request->conditions):"";
        if ($this->appClass->checkAccount($request->accountId)) {
            return new Response([
                'status' => 200,
                'data' => $this->appClass->getCheckupData($request->accountId, $request->trigger,$conditions),
    ...
    public function publishZap(Request $request)
    {
        ...
        if ($zap->save()) {
			...
			$this->updateUsedZaps(Auth::id(), 1);
            $secret = [$zap->id, 'Nit Published'];
            return json_encode([
                "status" => 200,
                "message" => Helpers::translate('Nit Created'),
    ...
    public function hasZaps($userId): bool
    {
        ...
        $zaps = $user_data['zaps'] ?? 0;
        $plan = $user_data['plan'] ?? 0;
        
        return $zaps;
    }

    public function updateZaps(int $userId, int $zapsAmount, int $totaltask){
        $user = User::find($userId);
        $user_data = json_decode($user->user_data, true);
        $zaps = $user_data['zaps'] ?? 0;
        $tasks = $user_data['tasks'] ?? 0;
        $updatedZap = $zaps + $zapsAmount;
        $updatedtasks = $tasks + $totaltask;
        $user_data['zaps'] = $updatedZap;
        $user_data['tasks'] = $updatedtasks;
        $user->user_data = json_encode($user_data, true);
        $user->save();

        return $updatedZap;
    }
	
	public function updateTask(int $userId, int $totaltask){
        $user = User::find($userId);
        $user_data = json_decode($user->user_data, true);
        $tasks = $user_data['tasks'] ?? 0;
        $updatedtasks = $tasks - $totaltask;
	...
	public function updateUsedZaps(int $userId, int $zapsAmount){
        $user = User::find($userId);
        $user_data = json_decode($user->user_data, true);
		$zaps = $user_data['zaps'] ?? 0;	
        $updatedZap = $zaps - $zapsAmount;
