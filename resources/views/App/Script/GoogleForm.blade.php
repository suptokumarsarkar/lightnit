<script>
    function getPersonalStatementGoogleForm(data) {
        return "<h6>" + data.name + "</h6><i style='font-size: 12px'>" + data.email + "</i>";
    }

    let yxform = 0;

    function connectAccountGoogleForm() {
        yxform = 0;
        regGoogleForm();

    }

    function connectAccountActionGoogleForm() {
        yxform = 1;
        regGoogleForm();
    }

    function checkTriggerGoogleForm(trigger, accountId, AppId = "GoogleForm") {
        $.ajax({
    ...
    function checkActionGoogleForm(trigger, action) {
        let data = {
            trigger: trigger,
            action: action
        }
...
<script>
    function regGoogleForm() {
        /*const clientGoogleForm = google.accounts.oauth2.initCodeClient({
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status === 200) {
                            displaySuccessToaster(data.message);
                            if (yxform) {
                                getAccountsAction('GoogleForm');
                            } else {
                                getAccounts('GoogleForm');
                            }
                        } else {
                            displayErrorToaster(data.message);
                        }
        ...
        clientGoogleForm.requestCode();
		*/
		
			
	 var authUrl = 'https://accounts.google.com/o/oauth2/auth?client_id={{$googleClientId}}&redirect_uri=https://lightnit.com/oauth/redirect&scope=profile%20email%20https://www.googleapis.com/auth/userinfo.email%20https://www.googleapis.com/auth/userinfo.profile%20https://www.googleapis.com/auth/drive.file%20https://www.googleapis.com/auth/drive&response_type=code&access_type=offline&prompt=consent';
