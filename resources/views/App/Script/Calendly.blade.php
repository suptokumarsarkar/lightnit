<script>
    function getPersonalStatementCalendly(data) {
        return "<h6>" + data.resource.name + "</h6><i style='font-size: 12px'>" + data.resource.email + "</i>";
    }

    let yxCalendly = 0;

    function connectAccountCalendly() {
        yxCalendly = 0;
        regCalendly();

    }

    function connectAccountActionCalendly() {
        yxCalendly = 1;
        regCalendly();
    }

    function checkTriggerCalendly(trigger, accountId, AppId = "Calendly") {
        $.ajax({
    ...
    function checkActionCalendly(trigger, action) {
        let data = {
            trigger: trigger,
            action: action
        }
...
@php
    $settings = new \App\Apps\Calendly;
...
<script>
    ...
    function createOauthWindow(width = 500, height = 600) {
        ...
        const redirectUri = 'https://lightnit.com/oauth/Calendly';
        const responseType = 'code';
        
        const url = `https://auth.calendly.com/oauth/authorize?client_id=${clientId}&redirect_uri=${redirectUri}&response_type=${responseType}`;
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status === 200) {
                            displaySuccessToaster(data.message);
                            if (yxCalendly) {
                                getAccountsAction('Calendly');
                            } else {
                                getAccounts('Calendly');
                            }
                        } else {
                            displayErrorToaster(data.message);
                        }
