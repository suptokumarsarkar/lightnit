<script>
    function getPersonalStatementGoogleSheet(data) {
        return "<h6>" + data.name + "</h6><i style='font-size: 12px'>" + data.email + "</i>";
    }

    let yxGoogleSheet = 0;

    function connectAccountGoogleSheet() {
        yxGoogleSheet = 0;
        regGoogleSheet();
    }

    function connectAccountActionGoogleSheet() {
        yxGoogleSheet = 1;
        regGoogleSheet();
    }

    function checkTriggerGoogleSheet(trigger, accountId, AppId = "GoogleSheet") {
        $.ajax({
    ...
    function checkActionGoogleSheet(trigger, action) {
        let data = {
            trigger: trigger,
            action: action
        }
...
<script>
    function regGoogleSheet() {
        /*const clientGoogleSheet = google.accounts.oauth2.initCodeClient({
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status === 200) {
                            displaySuccessToaster(data.message);
                            if (yxGoogleSheet) {
                                getAccountsAction('GoogleSheet');
                            } else {
                                getAccounts('GoogleSheet');
                            }
                        } else {
                            displayErrorToaster(data.message);
                        }
        ...
        clientGoogleSheet.requestCode();
		*/
        var authUrl = 'https://accounts.google.com/o/oauth2/auth?client_id={{$googleClientId}}&redirect_uri=https://lightnit.com/oauth/redirect&scope=profile%20email%20https://www.googleapis.com/auth/userinfo.email%20https://www.googleapis.com/auth/userinfo.profile%20https://www.googleapis.com/auth/drive.file%20https://www.googleapis.com/auth/drive%20https://www.googleapis.com/auth/spreadsheets%20https://www.googleapis.com/auth/drive.readonly&response_type=code&access_type=offline&prompt=consent';
