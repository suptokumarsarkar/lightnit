<script>
    function getPersonalStatementSlack(data) {
        return "<h6>" + data.accountname + "</h6><i style='font-size: 12px'>" + data.login.email + "</i>";
    }

    let yxSlack = 0;

    function connectAccountSlack() {
        yxSlack = 0;
        regSlack();

    }

    function connectAccountActionSlack() {
        yxSlack = 1;
        regSlack();
    }

    function checkTriggerSlack(trigger, accountId, AppId = "Slack") {
        $.ajax({
    ...
    function checkActionSlack(trigger, action) {
        let data = {
            trigger: trigger,
            action: action
        }
...
<script>
    ...
    function createOauthWindow(width = 500, height = 600) {
        ...
        const redirectUri = 'https://localhost/saas/oauth/Slack';
        const responseType = 'code';
        
        const scope = 'channels:read';
        
        const url = `https://the-jmb6013.slack.com/oauth?client_id=${clientId}&redirect_uri=${redirectUri}&response_type=${responseType}&scope=${scope}`;
