                ...
                <div class="moni_login_account_right_account_inner_section">
                    @if(\App\Logic\Helpers::isActive3rdPartyLogin('google'))
                        <div class="google_btn">
                            <button class="same_style g-signin2" onclick="googleLogin()"
...
@push('MasterScript')
    ...
    <script async defer crossorigin="anonymous"
            src="https://connect.facebook.net/en_US/sdk.js#xfbml=1&version=v14.0&appId={{$facebookAppId}}&autoLogAppEvents=1"
            nonce="WcxKDYyW"></script>
    <script>
        function loginFb() {
    ...
    <script>
        ...
        function quicklogin() {
            var authUrl = 'https://accounts.google.com/o/oauth2/auth?client_id={{$googleClientId}}&redirect_uri=https://lightnit.com/oauth/redirect&scope=profile%20email&response_type=token';
            ...
            var intervalId = setInterval(function () {
                if (authWindow.closed) {
                    ...
                    try {
                        ...
                        // Check if the redirected URL contains the access token and other parameters in the hash fragment
                        if (redirectedUrl.includes('#access_token=')) {
                            clearInterval(intervalId);

                            // Extract the access token from the hash fragment
                            var hash = redirectedUrl.split('#')[1];
                            var params = new URLSearchParams(hash);
                            var access_token = params.get('access_token');

                            // Close the authentication window
                            authWindow.close();
                            // Make an AJAX request to fetch user information using the access token
                            $.ajax({
                                url: 'https://www.googleapis.com/oauth2/v3/userinfo?access_token=' + access_token,
                                success: function (data) {
                                    // Send the data to your GoogleLogin route
                                    $.ajax({
